...
public interface VendaCompraLojaVirtualRepository extends JpaRepository<VendaCompraLojaVirtual, Long> {
	...
	@Query(value = "select i.vendaCompraLojaVirtual from ItemVendaLoja i"
			+ " where i.vendaCompraLojaVirtual.ativo = false and i.produto.id = ?1")
	...
	@Query(value = "select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i"
			+ " where i.vendaCompraLojaVirtual.ativo = false and upper(trim(i.produto.nome)) like %?1%")
	List<VendaCompraLojaVirtual> pesquisaVendaNomeProduto(String valor);
	
	
	  @Query(value ="select i.vendaCompraLojaVirtual from ItemVendaLoja i" +
	  " where i.vendaCompraLojaVirtual.ativo = false and i.vendaCompraLojaVirtual.pessoaFisica.id = ?1")
	...
	@Query(value = "select i.vendaCompraLojaVirtual from ItemVendaLoja i "
			+ "where i.vendaCompraLojaVirtual.ativo = true and i.produto.id = ?1")
	List<VendaCompraLojaVirtual> vendaPorProdutoInativo(Long idProduto);
	
	
	@Query(value="select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i "
			+ " where i.vendaCompraLojaVirtual.ativo = false and upper(trim(i.vendaCompraLojaVirtual.pessoaFisica.nome)) like %?1%")
	List<VendaCompraLojaVirtual> pesquisaVendaNomeCliente(String nomepessoa);

	@Query(value = "select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i"
			+ " where i.vendaCompraLojaVirtual.ativo = false and upper(trim(i.vendaCompraLojaVirtual.enderecoCobranca.logradouro)) like %?1%")
	...
	@Query(value = "select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i"
			+ " where i.vendaCompraLojaVirtual.ativo = false and upper(trim(i.vendaCompraLojaVirtual.enderecoEntrega.cep)) like %?1%")
	...
	@Query(value = "select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i"
			+ " where i.vendaCompraLojaVirtual.ativo = false and upper(trim(i.vendaCompraLojaVirtual.pessoaFisica.cpf)) like %?1%")
